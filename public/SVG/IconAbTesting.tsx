// icon:ab-testing | Material Design Icons https://materialdesignicons.com/ | Austin Andrews
import * as React from "react";

function IconAbTesting(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill={props.fill}
      height={props.height}
      width={props.width}
      {...props}
    >
      <path d="M4 2a2 2 0 00-2 2v8h2V8h2v4h2V4a2 2 0 00-2-2H4m0 2h2v2H4m18 9.5V14a2 2 0 00-2-2h-4v10h4a2 2 0 002-2v-1.5a1.54 1.54 0 00-1.5-1.5 1.54 1.54 0 001.5-1.5M20 20h-2v-2h2v2m0-4h-2v-2h2M5.79 21.61l-1.58-1.22 14-18 1.58 1.22z" />
    </svg>
  );
}

export default IconAbTesting;
